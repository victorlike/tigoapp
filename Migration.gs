...
function migrateSales() {
  ...
  const sales = data.map((row, index) => {
    ...
    return {
      ...
      vendedor_comentarios_at: getValue_(row, map, 'VendedorComentariosAt'),
      ...
      backoffice_at: getValue_(row, map, 'BackofficeAt'),
      ...
      bo_email_enviado_at: getValue_(row, map, 'BO_EmailEnviadoAt'),
...
function migrateLeadsFromSheet(sheetName) {
  ...
  const leads = data.map((row, index) => {
    ...
    return {
      ...
      seguimiento_tomado_en: getValue_(row, map, 'SeguimientoTomadoEn'),
      liberado_por: getValue_(row, map, 'LiberadoPor'),
      liberado_en: getValue_(row, map, 'LiberadoEn'),
